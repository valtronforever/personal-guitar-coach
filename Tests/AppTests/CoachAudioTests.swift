...
struct CoachAudioTests {
    ...
    @Test func vibratoExchangeHasItsOwnVersionTargetsAndExcludedObservations() throws {
        let root = FileManager.default.temporaryDirectory.appendingPathComponent("vibrato-coach-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
        let url = root.appendingPathComponent("audio.wav"); try wav(url)
        let event = try MusicalEvent(id: "motion", startTick: 0, durationTicks: 3840, kind: .note,
            positions: [FretPosition(string: 3, fret: 9)], vibrato: PitchVibrato(extentCents: 50, startTick: 960, endTick: 2880, periodTicks: 480))
        let endpoint = try CalibrationEndpoint(uid: "vibrato-coach", channel: 1, sampleRate: 48000, bufferFrames: 512)
        let config = try PracticeConfiguration(exercise: Exercise(id: "vibrato-coach", events: [event]), instrument: InstrumentProfile(), bpm: 60,
            route: CalibrationRoute(input: endpoint, output: endpoint, backendVersion: "fixture"))
        let start = try config.expectedStart(renderEpochSeconds: 100), hz = try Pitch(midi: 64).frequency()
        let trace = try PitchContourTrace(frames: (0..<211).map { try SustainFrame(id: UInt64($0 + 1), normalizedTime: start - 0.1 + Double($0) * 0.02, state: .pitched, frequency: hz) })
        let evidence = try PracticeEvidence(id: UUID(), configuration: config, startedAt: Date(timeIntervalSince1970: 1), finishedAt: Date(timeIntervalSince1970: 10),
            phase: .completed, reason: nil, signalConfirmed: true, renderEpochSeconds: 100, maximumClockDriftSeconds: 0,
            attacks: [PracticeAttack(id: 1, normalizedOnset: start, frequency: hz, clarity: 0.99, reliable: true),
                      PracticeAttack(id: 2, normalizedOnset: start + 1.2, frequency: nil, clarity: nil, reliable: false)],
        ...
        let result = try AssessmentEngine.evaluate(evidence)
        let request = try CoachExchange.prepare(audio: url, channel: 1, practice: result, language: "uk", lessonContext: "Hammer-on")
        #expect(request.promptVersion == "file-coach-6" && request.unscoredVibratoObservationIDs == [2])
        #expect(request.unscoredBendObservationIDs == nil)
        #expect(request.expectedNotes.first?.contains("vibratoWidthCents=50, vibratoRateHz=2.0") == true)
        #expect(ResultPresentation.annotations(result)["motion"] == .vibrato)
        #expect(FeedbackEngine.recommendations(for: result).contains { $0.kind == .vibrato && $0.eventIDs == ["motion"] })
        let response = CoachAnalysisResponse(schemaVersion: 1, provider: .codex, createdAt: Date(), request: request,
            feedback: CoachFeedback(schemaVersion: 1, requestID: request.id, language: "uk", summary: "Bend", findings: [CoachFinding(text: "Target", evidenceIDs: ["practice:motion"])]))
        let file = root.appendingPathComponent("response.json"), bytes = try CoachExchange.encode(response)
        try bytes.write(to: file)
        #expect(try CoachExchange.readResponse(file, practice: result).request.unscoredVibratoObservationIDs == [2])
        var object = try #require(JSONSerialization.jsonObject(with: bytes) as? [String: Any])
        var altered = try #require(object["request"] as? [String: Any]); altered["unscoredVibratoObservationIDs"] = []
        object["request"] = altered
        try JSONSerialization.data(withJSONObject: object).write(to: file)
        #expect(throws: CoachFileError.response) { try CoachExchange.readResponse(file, practice: result) }
    }
    @Test func chainExchangeHasItsOwnVersionTargetsAndExcludedObservations() throws {
        let root = FileManager.default.temporaryDirectory.appendingPathComponent("chain-coach-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
        let url = root.appendingPathComponent("audio.wav"); try wav(url)
        let event = try MusicalEvent(id: "motion", startTick: 0, durationTicks: 3840, kind: .note,
            positions: [FretPosition(string: 3, fret: 9)], legatoChain: LegatoChain(targets: [.init(kind: .hammerOn, semitones: 3, startTick: 960), .init(kind: .tap, semitones: 4, startTick: 1920), .init(kind: .pullOff, semitones: -7, startTick: 2880)]))
        let endpoint = try CalibrationEndpoint(uid: "chain-coach", channel: 1, sampleRate: 48000, bufferFrames: 512)
        let config = try PracticeConfiguration(exercise: Exercise(id: "chain-coach", events: [event]), instrument: InstrumentProfile(), bpm: 60,
            route: CalibrationRoute(input: endpoint, output: endpoint, backendVersion: "fixture"))
        let start = try config.expectedStart(renderEpochSeconds: 100), hz = try Pitch(midi: 64).frequency()
        let trace = try PitchContourTrace(frames: (0..<211).map { try SustainFrame(id: UInt64($0 + 1), normalizedTime: start - 0.1 + Double($0) * 0.02, state: .pitched, frequency: hz) })
        let evidence = try PracticeEvidence(id: UUID(), configuration: config, startedAt: Date(timeIntervalSince1970: 1), finishedAt: Date(timeIntervalSince1970: 10),
        ...
        let result = try AssessmentEngine.evaluate(evidence)
        let request = try CoachExchange.prepare(audio: url, channel: 1, practice: result, language: "uk", lessonContext: "Hammer-on")
        #expect(request.promptVersion == "file-coach-8" && request.unscoredLegatoChainObservationIDs == [2])
        #expect(request.unscoredBendObservationIDs == nil)
        #expect(request.expectedNotes.first?.contains("tap:tick=1920,pitch=B4") == true)
        #expect(ResultPresentation.annotations(result)["motion"] == .legatoChain)
        #expect(FeedbackEngine.recommendations(for: result).contains { $0.kind == .legatoChain && $0.eventIDs == ["motion"] })
        let response = CoachAnalysisResponse(schemaVersion: 1, provider: .codex, createdAt: Date(), request: request,
            feedback: CoachFeedback(schemaVersion: 1, requestID: request.id, language: "uk", summary: "Bend", findings: [CoachFinding(text: "Target", evidenceIDs: ["practice:motion"])]))
        let file = root.appendingPathComponent("response.json"), bytes = try CoachExchange.encode(response)
        try bytes.write(to: file)
        #expect(try CoachExchange.readResponse(file, practice: result).request.unscoredLegatoChainObservationIDs == [2])
        var object = try #require(JSONSerialization.jsonObject(with: bytes) as? [String: Any])
        var altered = try #require(object["request"] as? [String: Any]); altered["unscoredLegatoChainObservationIDs"] = []
        object["request"] = altered
        try JSONSerialization.data(withJSONObject: object).write(to: file)
        #expect(throws: CoachFileError.response) { try CoachExchange.readResponse(file, practice: result) }
    }
    @Test func fingerCueIsFrozenInCoachContextWithoutAClaimOfPhysicalDetection() throws {
        let root = FileManager.default.temporaryDirectory.appendingPathComponent("finger-coach-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
        let url = root.appendingPathComponent("audio.wav"); try wav(url)
        let event = try MusicalEvent(id: "motion", startTick: 0, durationTicks: 3840, kind: .note,
            positions: [FretPosition(string: 3, fret: 9)], pluckFinger: .middle)
        let endpoint = try CalibrationEndpoint(uid: "finger-coach", channel: 1, sampleRate: 48000, bufferFrames: 512)
        let config = try PracticeConfiguration(exercise: Exercise(id: "finger-coach", events: [event]), instrument: InstrumentProfile(), bpm: 60,
            route: CalibrationRoute(input: endpoint, output: endpoint, backendVersion: "fixture"))
        let start = try config.expectedStart(renderEpochSeconds: 100), hz = try Pitch(midi: 64).frequency()
        let trace = try PitchContourTrace(frames: (0..<211).map { try SustainFrame(id: UInt64($0 + 1), normalizedTime: start - 0.1 + Double($0) * 0.02, state: .pitched, frequency: hz) })
        let evidence = try PracticeEvidence(id: UUID(), configuration: config, startedAt: Date(timeIntervalSince1970: 1), finishedAt: Date(timeIntervalSince1970: 10),
        ...
        let result = try AssessmentEngine.evaluate(evidence)
        let request = try CoachExchange.prepare(audio: url, channel: 1, practice: result, language: "uk", lessonContext: "Hammer-on")
        #expect(request.promptVersion == "file-coach-9" && request.unscoredLegatoChainObservationIDs == nil)
        #expect(request.unscoredBendObservationIDs == nil)
        #expect(request.expectedNotes.first?.contains("pickingHandFinger=middle; authored cue, gesture unverified") == true)
        #expect(result.parameters == .current)
        #expect(result.evidence.configuration.exercise.events.first?.pluckFinger == .middle)
        let response = CoachAnalysisResponse(schemaVersion: 1, provider: .codex, createdAt: Date(), request: request,
            feedback: CoachFeedback(schemaVersion: 1, requestID: request.id, language: "uk", summary: "Bend", findings: [CoachFinding(text: "Target", evidenceIDs: ["practice:motion"])]))
        let file = root.appendingPathComponent("response.json"), bytes = try CoachExchange.encode(response)
        try bytes.write(to: file)
        #expect(try CoachExchange.readResponse(file, practice: result).request.unscoredLegatoChainObservationIDs == nil)
        var object = try #require(JSONSerialization.jsonObject(with: bytes) as? [String: Any])
        var altered = try #require(object["request"] as? [String: Any]); altered["promptVersion"] = "file-coach-1"
        object["request"] = altered
        try JSONSerialization.data(withJSONObject: object).write(to: file)
        #expect(throws: CoachFileError.response) { try CoachExchange.readResponse(file, practice: result) }
    }

    @Test func harmonicTargetsAndPhysicalInstructionsAreFrozenWithoutAGestureDetectionClaim() throws {
        let root = FileManager.default.temporaryDirectory.appendingPathComponent("harmonic-coach-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
        let url = root.appendingPathComponent("audio.wav"); try wav(url)
        let event = try MusicalEvent(id: "motion", startTick: 0, durationTicks: 3840, kind: .note,
            positions: [FretPosition(string: 3, fret: 7)], harmonic: HarmonicNote(kind: .natural, partial: 3))
        let endpoint = try CalibrationEndpoint(uid: "harmonic-coach", channel: 1, sampleRate: 48000, bufferFrames: 512)
        let config = try PracticeConfiguration(exercise: Exercise(id: "harmonic-coach", events: [event]), instrument: InstrumentProfile(), bpm: 60,
            route: CalibrationRoute(input: endpoint, output: endpoint, backendVersion: "fixture"))
        let start = try config.expectedStart(renderEpochSeconds: 100), hz = 195.99771799087463 * 3
        let trace = try PitchContourTrace(frames: (0..<211).map { try SustainFrame(id: UInt64($0 + 1), normalizedTime: start - 0.1 + Double($0) * 0.02, state: .pitched, frequency: hz) })
        let evidence = try PracticeEvidence(id: UUID(), configuration: config, startedAt: Date(timeIntervalSince1970: 1), finishedAt: Date(timeIntervalSince1970: 10),
            phase: .completed, reason: nil, signalConfirmed: true, renderEpochSeconds: 100, maximumClockDriftSeconds: 0,
            attacks: [PracticeAttack(id: 1, normalizedOnset: start, frequency: hz, clarity: 0.99, reliable: true),
            ...
            clipping: [], pitchContour: trace, analysisVersion: "fixture")
        let result = try AssessmentEngine.evaluate(evidence)
        let request = try CoachExchange.prepare(audio: url, channel: 1, practice: result, language: "uk", lessonContext: "Natural harmonic")
        #expect(request.promptVersion == "file-coach-10" && request.unscoredLegatoChainObservationIDs == nil)
        #expect(request.unscoredBendObservationIDs == nil)
        #expect(request.expectedNotes.first?.contains("harmonic=natural, partial=3, string=3, baseFret=0, touchNearFret=7") == true)
        #expect(result.parameters == .withHarmonics)
        #expect(request.expectedNotes[0].contains("D5") && request.expectedNotes[0].contains("physical production and timbre unverified"))
        #expect(abs(result.notes[0].targetFrequency - 587.9931539726239) < 1e-9)
        #expect(result.evidence.configuration.exercise.events.first?.harmonic?.partial == 3)
        let response = CoachAnalysisResponse(schemaVersion: 1, provider: .codex, createdAt: Date(), request: request,
            feedback: CoachFeedback(schemaVersion: 1, requestID: request.id, language: "uk", summary: "Harmonic", findings: [CoachFinding(text: "Target", evidenceIDs: ["practice:motion"])]))
        let file = root.appendingPathComponent("response.json"), bytes = try CoachExchange.encode(response)
        try bytes.write(to: file)
        #expect(try CoachExchange.readResponse(file, practice: result).request.unscoredLegatoChainObservationIDs == nil)
        var object = try #require(JSONSerialization.jsonObject(with: bytes) as? [String: Any])
