...
@MainActor struct PitchTransitionRenderTests {
    @Test func renderTransitionNotationWhenRequested() throws {
        guard let directory = ProcessInfo.processInfo.environment["COACH_TRANSITION_RENDER_DIR"] else { return }
        ...
        let kinds = PitchTransition.Kind.allCases
        var events: [MusicalEvent] = []
        for (index, kind) in kinds.enumerated() {
            let start = Int64(index) * 3840
            events.append(try MusicalEvent(id: kind.rawValue, startTick: start, durationTicks: 2880, kind: .note,
                positions: [FretPosition(string: 3, fret: 7)], pickStroke: .down,
                pitchTransition: PitchTransition(kind: kind, semitones: kind == .pullOff ? -2 : 2, startTick: 960, travelTicks: kind == .slide ? 960 : 0)))
            events.append(try MusicalEvent(id: "rest-\(index)", startTick: start + 2880, durationTicks: 960, kind: .rest))
        }
        let model = try TimelineModel(exercise: Exercise(id: "motion-render", events: events), instrument: .cStandard)
        for language in ["en", "uk"] {
            ...
            for dark in [false, true] {
                let view = TransitionRenderFixture(model: model, bundle: bundle)
                ...
                try #require(bitmap.representation(using: .png, properties: [:])).write(to: root.appendingPathComponent("transition-\(language)-\(dark ? "dark" : "light").png"))
            }
        }
    }
}

private struct TransitionRenderFixture: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            ForEach(0..<layout.rowCount, id: \.self) { row in
                PracticeScoreRow(model: model, layout: layout, row: row, cursor: nil, selectedIDs: ["slide"],
            ...
            if let event = model.events.last(where: { $0.event.pitchTransition != nil })?.event {
