...
@MainActor struct StarterCourseFlowTests {
    @Test func EachBundledLessonConnectsTextVisualsPracticeSavedResultAndRetry() async throws {
        ...
        #expect(library.issues.isEmpty && library.lessons.count == 94)
        ...
        for lesson in library.lessons {
            ...
            for entry in lesson.manifest.practiceEntries {
                let snapshot = try lesson.resolveActivity(id: entry.activityID, instrument: InstrumentProfile())
                ...
                let contour: PitchContourTrace? = try configuration.selectedEvents.contains(where: { $0.bend != nil || $0.pitchTransition != nil || $0.vibrato != nil }) ? PitchContourTrace(frames:
                    (0...Int(ceil((end - start + 0.4) / 0.02))).map { i in
                        try SustainFrame(id: UInt64(i + 1), normalizedTime: start - 0.1 + Double(i) * 0.02, state: .silence, frequency: nil)
                    }) : nil
            ...
            for exercise in lesson.manifest.exercises where exercise.assessmentMode == .displayOnly {
                #expect(PracticeRequest(lesson: lesson, snapshot: try lesson.resolveActivity(id: lesson.manifest.activities[0].id, instrument: InstrumentProfile()), entryID: exercise.id) == nil)
            }
        }
        await data.reload()
