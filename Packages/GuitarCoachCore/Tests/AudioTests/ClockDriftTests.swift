...
struct ClockDriftTests {
    private func input(frame: UInt64, packet: UInt32 = 512, drift: Double = 0) -> CaptureSnapshot {
        CaptureSnapshot(totalFrames: frame + UInt64(packet), totalPackets: frame / UInt64(packet) + 1, droppedPackets: 0,
            peak: 0.1, rms: 0.05, sampleRate: 48000, lastHostTime: AVAudioTime.hostTime(forSeconds: 100 + Double(frame) / 48000 + drift),
            hostTimeValid: true, lastPacketFrames: packet)
    }
