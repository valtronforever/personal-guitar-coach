...
struct MixedPitchMotionTests {
    @Test(arguments: [false, true]) func movingNotesShareWeightBeforeSustainAndRoundTripWithoutRescoring(addVibrato: Bool) throws {
        ...
        if addVibrato {
            events.append(try MusicalEvent(id: "vibrato", startTick: 11520, durationTicks: 3840, kind: .note, positions: [position],
                vibrato: PitchVibrato(extentCents: 80, startTick: 960, endTick: 2880, periodTicks: 480)))
        }
        ...
        let contour = try (0..<(addVibrato ? 811 : 611)).map { i -> SustainFrame in
        ...
        let attacks = try (addVibrato ? [0.0,4,8,12] : [0.0,4,8]).enumerated().map { try PracticeAttack(id: UInt64($0.offset + 1), normalizedOnset: start + $0.element, frequency: hz, clarity: 0.99, reliable: true) }
        ...
        #expect(result.overallScore == (addVibrato ? 89 : 90) && result.parameters == (addVibrato ? .withVibrato : .withPitchTransitions))
        if addVibrato { #expect(abs((result.vibratoScore ?? 0) - 200.0 / 3) < 1e-7) }
