...
struct ElectricStylesCourseTests {
    @Test func originalStyleFormsRetainIndependentMusicAcrossEveryPresetAndNeck() throws {
        ...
        let targets: [String: [String: [Int]]] = [
            "blues-accompaniment": ["chord-map": bluesChords, "shuffle-cell": shuffle,
                "shuffle-form": bluesRoute.flatMap { n in shuffle.map { $0+n } }, "ending": [7,5,0,0].flatMap { n in [58+n,64+n,67+n] }],
            "blues-solo": ["plain-call": call, "bent-call": call, "vibrato-answer": answer,
                "twelve-bar-solo": call + [60] + answer + call + [65,63,60] + answer + [67,70,67,65,63,60] + answer + [60]],
            "classic-rock": ["main-riff": rock+rock, "rhythm-part": rockChords+rockChords, "one-bar-fill": fill,
                "eight-bar-arrangement": rock+rock+rockChords+rockChords+rock+rock+fill+c],
            "punk-hardcore": ["steady-eighths": repeated(c,8), "hard-stops": stops, "broad-pulse": c+c+f+f,
                "eight-bar-form": repeated(c,8)+repeated(f,8)+repeated(ab,8)+repeated(bb,4)+stops],
            "metal-study": ["low-pedal": repeated([40],8)+[43,45,40], "gallop-burst": repeated([40],12),
                "tuning-shapes": e5+g5+a5+e5, "four-bar-riff": repeated([40],16)+g5+a5+repeated([40],12)+g5+a5+e5],
            "rnb-neo-soul": ["color-chords": cm7+am7+dm7+g9, "top-line": [71,67,69,69],
                "inversion-choice": [60,64,67,64,67,72], "ornament-answer": cm7+[64]+dm7+[65],
                "color-study": cm7+[64]+am7+dm7+[65]+g9+cm7],
            "country-chicken-picking": ["pick-finger-exchange": country, "short-upper-notes": [64,67,69,67,64,62,60,60],
