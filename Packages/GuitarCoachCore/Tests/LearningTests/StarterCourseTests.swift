...
struct StarterCourseTests {
    ...
    @Test func SixBilingualLessonsMatchIndependentMusicalSequencesAndDeclaredTempos() throws {
        ...
        for (lesson, reference) in zip(lessons, expected) {
            ...
            #expect(exercise.tuningPolicy == .fixedTuning && exercise.requiredTuning == .standard)
            for bpm in [exercise.minimumBPM, exercise.defaultBPM, exercise.maximumBPM] {
                #expect(try MonophonicCapability.limitations(exercise: exercise, instrument: .standard, bpm: bpm).isEmpty)
                try exercise.validateForPractice(instrument: .standard, bpm: bpm)
            }
        ...
        for lesson in try load() {
            for exercise in lesson.manifest.exercises where lesson.manifest.practiceExerciseIDs.contains(exercise.id) {
                for rate in [44100.0, 48000.0] {
                    ...
                    let config = try PracticeConfiguration(exercise: exercise, instrument: InstrumentProfile(), bpm: exercise.defaultBPM,
                    ...
                    for event in config.selectedEvents where event.kind == .note {
                        ...
                        let frequency = try TuningProfile.standard.frequency(at: event.positions[0])
