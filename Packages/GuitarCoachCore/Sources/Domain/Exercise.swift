...
public struct MusicalEvent: Hashable, Codable, Identifiable, Sendable {
    ...
    public let pluckFinger: PluckingFinger?
    public var pickingDirection: StrumDirection? { strum?.direction ?? pickStroke ?? mutedAttack?.direction }
    ...
    public init(id: String, startTick: Int64, durationTicks: Int64, kind: MusicalEventKind, positions: [FretPosition] = [], assessSustain: Bool = false, accented: Bool = false, strum: StrumPattern? = nil, palmMuted: Bool = false, pickStroke: StrumDirection? = nil, bend: PitchBend? = nil, pitchTransition: PitchTransition? = nil, vibrato: PitchVibrato? = nil, legatoChain: LegatoChain? = nil, pluckFinger: PluckingFinger? = nil, harmonic: HarmonicNote? = nil, mutedAttack: MutedStringAttack? = nil, heldStrings: [Int] = []) throws {
        ...
        guard (kind == .rest && positions.isEmpty && mutedAttack == nil) || (kind == .note && ((!positions.isEmpty && mutedAttack == nil) || (positions.isEmpty && mutedAttack != nil))),
        ...
        if let bend {
            guard kind == .note, positions.count == 1, positions[0].fret > 0, !assessSustain, !palmMuted else { throw MusicError.invalidEvent }
            try bend.validate(durationTicks: durationTicks)
        }
        if let pitchTransition {
        ...
        if let legatoChain {
            guard kind == .note, positions.count == 1, bend == nil, pitchTransition == nil, vibrato == nil, !assessSustain, !palmMuted else { throw MusicError.invalidEvent }
    ...
    public func encode(to encoder: Encoder) throws {
        ...
        try values.encodeIfPresent(pluckFinger, forKey: .pluckFinger)
...
public struct ResolvedEvent: Hashable, Sendable, Identifiable {
    ...
    public var transitionTargetPitch: Pitch? {
        guard let transition = event.pitchTransition, let pitch = pitches.first else { return nil }
        return try? Pitch(midi: pitch.midi + transition.semitones)
    }
}
