...
public struct PracticeActivityReference: Codable, Equatable, Sendable {
    ...
    public init(activityID: String, materialID: String, entryID: String, choice: PositionChoice,
                ...
                selfConfirmation: PositionSelfConfirmation? = nil, presentation: PracticePresentation? = nil) throws {
        self.presentation = presentation; schemaVersion = presentation == nil ? 1 : 2; self.activityID = activityID; self.materialID = materialID; self.entryID = entryID
    ...
    public func validate() throws {
        ...
        guard (schemaVersion == 1 && presentation == nil || schemaVersion == 2 && presentation != nil), [activityID, materialID, entryID, resolverVersion, source.exerciseID].allSatisfy(validID),
        ...
        if presentation == .listenAndRepeat {
            guard !positioning.enabled && choice == .original && selfConfirmation == nil else { throw PracticeError.invalidEvidence }
        }
    ...
    public func validate(exercise: Exercise) throws {
        try validate()
        guard presentation == nil || exercise.assessmentMode == .monophonic else { throw PracticeError.invalidEvidence }
    ...
    public func hasSameConditions(as other: Self) -> Bool {
        activityID == other.activityID && materialID == other.materialID && entryID == other.entryID && choice == other.choice
            && presentation == other.presentation && positioning == other.positioning && requiredChoice == other.requiredChoice && resolverVersion == other.resolverVersion && source == other.source
    }
    public func withoutSelfConfirmation() throws -> Self {
        try Self(activityID: activityID, materialID: materialID, entryID: entryID, choice: choice, positioning: positioning,
            requiredChoice: requiredChoice, resolverVersion: resolverVersion, source: source, lessonTitles: lessonTitles, activityTitles: activityTitles, presentation: presentation)
    }
    private enum CodingKeys: String, CodingKey {
        case schemaVersion, activityID, materialID, entryID, choice, positioning, requiredChoice, resolverVersion, source, lessonTitles, activityTitles, selfConfirmation, presentation
    }
    public init(from decoder: Decoder) throws {
        let v = try decoder.container(keyedBy: CodingKeys.self)
        let decodedVersion = try v.decode(Int.self, forKey: .schemaVersion)
        try self.init(activityID: v.decode(String.self, forKey: .activityID), materialID: v.decode(String.self, forKey: .materialID),
            ...
            presentation: v.decodeIfPresent(PracticePresentation.self, forKey: .presentation))
        guard decodedVersion == schemaVersion else { throw PracticeError.invalidEvidence }
    }
}
