...
public struct PracticeConfiguration: Codable, Equatable, Sendable {
    ...
    public init(exercise: Exercise, instrument: InstrumentProfile, bpm: Double, range: Range<Int64>? = nil,
        ...
        try self.init(exercise: exercise, instrument: instrument, bpm: bpm, range: range, countInBars: countInBars,
            route: route, calibration: calibration, outputAlignment: outputAlignment, lesson: lesson, listeningConditions: listeningConditions, capabilityVersion: exercise.events.contains(where: { $0.harmonic != nil && (range?.contains($0.startTick) ?? true) }) ? MonophonicCapability.harmonicVersion : exercise.events.contains(where: { $0.legatoChain != nil && (range?.contains($0.startTick) ?? true) }) ? MonophonicCapability.legatoChainVersion : exercise.events.contains(where: { $0.vibrato != nil && (range?.contains($0.startTick) ?? true) }) ? MonophonicCapability.vibratoVersion : exercise.events.contains(where: { $0.pitchTransition != nil && (range?.contains($0.startTick) ?? true) }) ? MonophonicCapability.pitchTransitionVersion : MonophonicCapability.version,
