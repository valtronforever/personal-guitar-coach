...
public enum CoachAgentContract {
    ...
    public static func prompt(requestJSON: String, audioPath: String) -> String {
        ...
        Picking-hand finger cues are authored instructions, independent of the player’s handedness. They do not prove which finger, nail or pick contacted a string. For hybrid/economy/string-skipping tasks, separate measured pitch/time evidence from physical self-observation.
        Harmonic metadata separates physical base/touch locations from sounding pitch. Use the frozen exact targetFrequency (ideal open or stopped fundamental times the partial), not ordinary fret arithmetic. A correct harmonic pitch does not prove a light touch, artificial/pinch gesture or harmonic timbre. Natural fret numbers are approximate touch landmarks, not stopped notes.
        ...
        For assessmentMode rhythmOnly (repeated-attack-assessment-1), assess only attack timing, misses and extra onsets. pitchScore and centsError are intentionally absent. Periodic windows only reject unsuitable input; they do not verify the note, timbre, alternating pick direction or technique. Never fill the missing pitch score from offline analysis or infer successful pitch matching. When rhythmOnly validity is uncalibrated, event/attack assignments are diagnostic only: do not infer missed notes, extra notes or rest errors from them. Recommend calibration and a neutral retry instead.
