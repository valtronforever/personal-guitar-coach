...
public enum FeedbackKind: String, Sendable {
    case inputLevel, signal, interrupted, restart, calibration, early, late, missed, pitch, tuning, rests, sustain, bend, pitchTransition, vibrato, legatoChain, repeatFragment
}
...
public enum FeedbackEngine {
    public static func recommendations(for result: AssessedPractice) -> [PracticeRecommendation] {
        ...
        if result.validity == .insufficientSignal {
            ...
            if let chains = result.legatoChains { uncertainIDs.formUnion(chains.notes.filter { $0.score == nil }.map(\.id)) }
        ...
        let chainIDs = Set(result.legatoChains?.notes.filter { $0.phases.contains { ($0.matchedFraction ?? 1) < 0.8 } }.map(\.id) ?? [])
        if let chain = best(result.notes.filter { chainIDs.contains($0.id) }, minimum: 1, kind: .legatoChain) { answer.append(chain) }
...
public enum PracticeComparison {
    ...
    public static func compatible(_ lhs: AssessedPractice, _ rhs: AssessedPractice) -> Bool {
        ...
        return lhs.validity == rhs.validity && (lhs.validity == .valid || lhs.validity == .uncalibrated)
            ...
            && a.lesson?.position == b.lesson?.position && a.listeningConditions == b.listeningConditions
