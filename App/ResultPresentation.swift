...
enum ResultAnnotation: String, CaseIterable {
    case uncertain, missing, pitch, early, late, sustain, bend, pitchTransition, matched, restExtra
...
enum ResultPresentation {
    static func annotations(_ result: AssessedPractice) -> [String: ResultAnnotation] {
        ...
        for note in result.notes {
            if transitions[note.id].map({ $0.score == nil }) == true || note.uncertain || sustain[note.id]?.state == .uncertain || bends[note.id].map({ $0.score == nil }) == true { annotations[note.id] = .uncertain }
