...
### Listen-and-repeat preparation

The optional `listenAndRepeat` presentation uses the existing output-only preview transport, followed by the ordinary input preflight/count-in/practice pipeline. Both transports use the same resolved exercise, tempo and bar range. The teacher tone is disabled during the response, and no recording/capture starts during the reference. After output completion, preparation waits for the configured/reported output allowance before enabling a response. This does not measure headphone latency; it uses the existing output setting. Coordinator preview completion releases output ownership but retains a completed snapshot, which the preparation state recognizes by request ID, idle phase and route revision. Canceled/stale playback cannot prepare a later selection.

`PracticeListeningConditions` version 1 freezes completed-reference and revealed-target flags. Hidden targets require a completed reference; deliberate reveal permits guided practice. Conditions must be present exactly when the activity's optional presentation requires them. Schema 1 activity references remain readable and omit presentation; opted-in references use schema 2. The acoustic matching algorithm is unchanged: notes, onset timing, valid-signal and calibration requirements still apply. Result comparisons additionally require equal listening conditions. Software completion and hidden notation do not certify hearing, memory, unfamiliarity or an ear-training skill score. The reference is not evidence of the learner's guitar performance.

### Legato-chain assessment (software verification; hardware pending)

`LegatoChain` adds 1–8 event-local abrupt pitch changes without new attack targets. `legato-chain-capability-1` requires a 195.9–880 Hz initial pitch, all target plateaus 195.9–1100 Hz and ≥0.4 s per plateau using the actual meter pulse. The existing worker contour collector is reused; no new capture pipeline or audio callback work is introduced. Reference rendering caches cumulative piecewise phase integrals at event construction and uses binary search without per-sample allocation/pow; seek and loop preserve the phase accumulated before each target.

`LegatoChainEvaluator` aligns all targets to the observed initial attack, or expected time plus archived input correction if no attack was matched. Each plateau measures duration-weighted ±35-cent coverage after 200 ms of initial settling or 60 ms at later boundaries, ending 60 ms before the next change. Known silence and known wrong pitch score differently from >20% unknown coverage, which prevents a valid grade. Internal pitch-motion observations from the first target minus 60 ms through event end remain archived but are excluded from extra-pick penalties; sound cannot prove repicking or physical hammer/pull/tap gestures. Actual initial timing is compensated once.

Opt-in `mono-capability-6`, `monophonic-assessment-7`, `legato-chain-assessment-1` and frozen cumulative offsets distinguish new attempts from old history. Mixed bend/transition/vibrato/chain exercises use the per-note moving-score mean with the existing 50/50 attack/motion combination (then optional 80/20 sustain). `file-coach-8` carries every target pitch/tick and `unscoredLegatoChainObservationIDs`, validated on response; the prompt prohibits inferring physical gestures. Offline/synthetic evidence does not establish real guitar, route, latency or provider validation.

Picking-hand `pluckFinger` is an authored cue only. It does not modify MonophonicAnalyzer, capture, transport frequencies/envelopes, matching or scoring parameters. A differential test verifies identical reference samples and note assessment for absent/p/i/m/a cues, while frozen practice encoding retains the chosen instruction. `file-coach-9` includes the expected picking-hand role and explicitly labels it unverified; the existing digest binds the instruction to the attempt. No string-skipping, economy, sweep or hybrid hand gesture is inferred from pitch/onset observations.

## Harmonic targets (topic 94)

For natural partials 2/3/4 and touched artificial octaves, target/reference frequency is the tuned open or stopped base frequency times the partial. The nearest MIDI name is only a label; the third partial is not rounded to equal temperament for scoring. The existing detector remains `mono-mpm-flux-4`. Selected harmonic attempts use `mono-capability-7` and `monophonic-assessment-8`; ordinary attempts retain their earlier parameters. Exact target frequencies are validated on decoding. Optional existing sustain assessment uses that same target.

Synthetic PCM at 44.1/48 kHz exercises the existing collector, pitch/onset assessment and frozen results. Such evidence does not establish real guitar harmonic reliability, mixed residual fundamentals, pickup response or technique recognition. Physical hand/node use and harmonic timbre are unverified. No new capture pipeline or callback allocation is introduced; expected tones remain silent during measured practice. `file-coach-10` freezes harmonic kind, partial, string, base/touch positions and exact Hz, and explicitly withholds gesture/timbre claims.

## Fully muted-string attacks

`mutedAttack` represents an authored unpitched percussive sound. It has no pitch target and is restricted to display/reference/self-practice; pure and mixed exercises reject monophonic grading at construction. Unlike `palmMuted`, it does not retain a fundamental. The reference is deterministic short decaying noise (up to 80 ms, clipped to the written event end), addressed by age from the original event onset. Chunk boundaries, seeks and loops do not retrigger an existing attack. Rest events remain silent, and practice transport does not emit reference tones/noise. No noisy-attack detector, hand/damping assessment or new capture pipeline is introduced.

## Held-voice references (display only)

Plain polyphonic examples can carry held-string continuations. The pure Domain model validates each tie and merges complete voice spans; TransportPlan prepares their frequencies outside the callback. Rendering uses each voice's original onset and final end, a stable maximum-active-voice divisor and original attack accent. Internal event boundaries do not restart phase, attack envelope or volume. Chunk/seek/loop behavior is covered by an independent three-voice analytic signal at 44.1/48 kHz. A seek or selected range clips the requested output; no new capture pipeline, DSP detector or grading capability is introduced. Practice-mode reference output remains silent. The display-only gate and explicit polyphonic staff fallback remain in force; real instrument continuity and physical fingering require user acceptance.

## Rapid fixed-note rhythm contract (2026-09-17)

An explicit `rhythmOnly` exercise supports one repeated clean pitch at 110–440 Hz, minimum 0.15 s per note. The capture callback/pipeline and `mono-mpm-flux-4` analyzer are unchanged. The existing worker's detected-onset stream supplies timestamps, even when subsequent attacks prevented a settled note pitch. Practice now collects its separate bounded periodic-window trace for this mode. At least two mutually consistent periodic frames within the first 125 ms of each attack are required; they are a signal-quality gate, never a target-pitch match. Clipping, missing trace, too few frames and ambiguous/noisy input remain uncertain. Known silence after valid preflight remains a missed-performance result. Missing targets with uncertain signal do not become healthy silence. Capture gaps, format/route changes and invalid samples retain the coordinator/collector failure path.

Timing uses the existing time-only sequence alignment, rest boundaries, count-in exclusion, drift/calibration checks and extra-onset penalty. The new assessment version omits pitch/cents completely; it cannot be encoded as an ordinary pitched result or stored with a mismatched exercise mode. Ordinary note duration/pitch rules remain unchanged. The rapid onset allowance is 20 ms for the bounded clean synthetic corpus, not measured Scarlett/Bluetooth performance. Calibration uncertainty plus onset allowance plus drift must still fit half the exercise's tolerance; rough manual settings often withhold fast timing. The UI explains that case and offers slower-tempo/synchronization guidance. No automatic pitch fallback or fabricated score is allowed.

Real clean-guitar bursts, envelope/pick differences, noise, monitoring leakage, actual route latency and accessibility remain pending user validation. This mode measures audible attack timing, never pick direction, finger/string identity, dynamics or tremolo technique correctness.
