...
Both settings accept **0–1000 ms**, with a decimal dot/comma; their combined compensation must not exceed 1000 ms. Negative additional corrections are also prohibited, per the user's 2026-09-16 decision. Output has two references: **additional correction** or **total delay from specification**. The latter must be at least the output's already-reported hardware latency, since subtracting that latency must not produce a negative correction. Unknown reported latency is zero. The original entered total and reference remain stored; actual Bluetooth connections may differ from published specifications.

This is a setting-selection policy, not a rewrite of signed timing evidence. Parser, model apply actions and store writes reject negative settings, including measured candidates. A negative measurement remains visible on the diagnostic timeline, with Apply disabled and a localized explanation. Existing signed records remain readable with their original schemas/versions; current selection excludes negative output settings (explicitly showing the zero fallback) and negative instrument settings (ineligible for scoring). Users can reset/replace them. Historical practice snapshots and scores are not rewritten.
...
Normalization subtracts reported input hardware latency from observed host timestamps and adds reported output hardware latency to rendered click times. Neither reported latency nor the output setting is subtracted again in assessment. Signed diagnostic offsets and historical snapshots are retained; only nonnegative settings may be selected for new practice. Constant personal anticipation/lateness remains inseparable from audio delay.
...
Practice cursor and instrument timeline subtract reported output latency plus the independently saved output alignment (0–1 second for newly selected manual entries). They never use total guitar compensation as a display delay. Output-only alignment can move the practice cursor even when rhythm scoring is unavailable. When endpoint hardware timing is unknown, a measured output offset already includes the unknown presentation delay: the cursor does not add the renderer fallback again. Before an output setting exists, practice retains its renderer-presentation fallback. Calibration timelines always use the same hardware-or-zero reference as their expected timestamps.
