...
## Low-register follow-up (2026-09-16)

Current algorithm: `mono-mpm-flux-4`, capability `mono-capability-2`. A1–E6 targets are now allowed with the same 55–1500 Hz actual-frequency gate. An exploratory low-note repeat matrix exposed three A1 fixtures that resolved sustained pitch but lost later attacks: the new transient had entered the eight-hop background before its flux peak. The background now uses eight older hops, excluding the two latest hops (20 ms). Threshold constants, falling-signal guard, refractory time and startup time are unchanged. Processing remains on the existing worker, not the real-time callback.

[Full regression evidence](../benchmarks/low-register-audio.json): 4,476 MPM/YIN cases, including 540 MPM low-register cases across MIDI 33–36, A4 400/440/480 where within range, three attack phases, three harmonic profiles, 200/300/500 ms notes and both input rates. All 2,160 low-register attacks resolved with no missed/extra events; onset p95 11.12 ms, stable pitch p95 0.108 cents. The recorded development corpus still passes existing gates (two conservatively counted extra transients). Tests also cover long low sustains, 50 Hz rejection, and PCM through collector to assessment. These results do not establish physical guitar/pickup or hardware latency performance.
