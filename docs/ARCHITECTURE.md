...
AI exchange retains full local immutable evidence and its digest in a bounded 16 MB request /18 MB saved response envelope. Before invoking a CLI, AgentBridge strips the raw derived sustain trace from the prompt and retains per-note sustain summaries and algorithm/validity metadata. CLI output remains bounded at 2 MB. No additional recordings or automatic AI requests are introduced.
