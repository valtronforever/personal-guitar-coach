...
`CalibrationRoute` і `CalibrationProfile` у Domain визначають host normalization та rhythm capability. `OutputAlignmentProfile` — окрема preference виходу (відсутність = 0); `InstrumentSyncEvidence` фіксує її snapshot і залишковий зсув інструменту; `ManualOutputAlignment` та `ManualInstrumentSyncEvidence` зберігають окреме ручне походження, а `manualPersonal` дає явно приблизне оцінювання за вибором користувача. Audio публікує output endpoint без вимоги input, рендерить 4+16 кліків у preview для натискань або calibration capture для гітари. Persistence атомарно зберігає output-alignment schema 1 та calibration schema 3 (читає 1/2/3). Спільний `CalibrationStore` володіє `CalibrationModel` і утримує дві діагностичні шкали між відкриттями sheet у поточній сесії. Capture/transport UUID lease захищає start/stop. `PracticeConfiguration` фіксує окремо output alignment для курсора й сумарний instrument profile для оцінювання без подвійної компенсації. Personal receipt живе лише в поточному аудіосеансі/route revision й потребує незмінного output setting. Деталі — [синхронізація](PERSONAL-SYNCHRONIZATION.md) та [ADR 003](decisions/003-calibration-time.md).
...
Domain defines `PositionChoice`, `FretRegion` and validated `PositioningPolicy` (width 1–13, auto/list/range, optional Original). Learning's `resolveActivity` selects a material, establishes tuning-adapted sounding pitches, resolves positions and renders both locales into one immutable snapshot. Materials cover a lesson, exercise, contiguous event fragment or named fingering. Activities share source music with independent learner/fixed choices. Whole-material availability includes linked shapes and arpeggios; no octave substitution is allowed.
...
## Course organization (full curriculum, stage 1)

`LessonModule` and optional `LessonCurriculumPlacement` add bilingual catalog grouping, global order, estimated visit duration, advisory prerequisite IDs and search keywords. They do not change exercise identity or historic assessments. Loader validation isolates invalid placements and rejects dependency cycles. `LessonTool` carries explicit tuner/audio/settings links through activity resolution; views do not infer destinations from prose.

`LessonLibraryQuery` combines bilingual word search with module, difficulty, topic, learning mode and version-aware reading status, then sorts deterministically. `LessonLibraryStore` supplies ordered groups, next available lesson and continuation. The app opens the course overview rather than automatically navigating away; Continue restores the saved lesson/step. Legacy tuning aliases remain loadable for history and hidden from the course. Course metadata and authored text coverage are tracked separately from grading capabilities in `docs/curriculum/coverage.yml`.

## Rhythm fragments and sustained-note evidence

`RhythmNotation` projects each timeline segment into identified written fragments `(eventID,startTick)`. Staff selection and keyboard focus distinguish fragments while all musical actions retain canonical event IDs. Ties carry pitch/accidentals across barlines without introducing new events. Preview/transport never sees a second attack. `PracticeBarSelection` expands a selected or resumed range to complete events, including rest spans, and reports expansion visibly.

Domain owns the additive `assessSustain` opt-in, validated `SustainTrace` and archived `SustainAssessment`. Audio produces a bounded timestamped trace on its analysis worker; PracticeEvidenceCollector retains it only for opted-in exercises. Learning computes coverage and versioned feedback; views display archived results without alternate pitch/timing calculations. Missing trace evidence is a distinct quality failure. See the numerical contract in AUDIO-AND-ASSESSMENT.md.

AI exchange retains full local immutable evidence and its digest in a bounded local envelope (expanded to 24 MB request /28 MB saved response for mixed bend/sustain traces below). Before invoking a CLI, AgentBridge strips the raw derived sustain and pitch-contour traces from the prompt and retains per-note sustain summaries and algorithm/validity metadata. CLI output remains bounded at 2 MB. No additional recordings or automatic AI requests are introduced.

`MusicalEvent.accented` is presentation/reference-playback metadata. Domain rejects accented rests; Learning preserves the flag during adaptation. Staff fragments expose the accent only at the canonical attack. Normal and practice TAB announce/render it without changing event selection or timing. TransportPlan changes reference amplitude (1.5×), never scheduled frames, and the existing practice-mode guard suppresses all reference tones. Assessment has no loudness or picking-technique inference. Default-false encoding preserves older request digests.

Materials can declare a source `tonalRoot: Pitch` for interval-preserving lessons. Loader validation requires explicit compatible source tuning. Activity resolution applies the same uniform transposition as sounding pitches and passes the independent root to text rendering. This removes the naming dependency on the sorted/first chord voice without changing `first`, fingering positions, audio targets, or historic titles. Materials without this optional field keep their previous rendering. The initial minor/major chord lessons use distinct shape and arpeggio activities; polyphonic shape snapshots remain display-only.

Strum references use a validated optional `MusicalEvent.strum` (direction plus PPQ spread). The audio worker precomputes voice offsets ordered by actual string numbers and renders each voice from its own absolute scheduled onset, ending at the original event boundary. Seeking into the chord preserves voice age rather than replaying the strum; absolute sample rounding can change phase by at most one sample at 44.1 kHz. Capture/assessment/click timing is unchanged. TAB and compact rows show direction only on the attack segment; accessibility also names direction and emphasis. Polyphonic staff remains unsupported explicitly.

Lesson transposition now has an author-controlled anchor string (default 1). One pure `LessonAdaptationDefinition.transposition` computes the shift shared by event/fingering resolution and explicit harmonic-root labels. Anchor 6 supports open-bass Standard/Drop riffs without distorting power-chord intervals; existing lessons keep their previous default. The resolved exercise still freezes all actual pitches/timing through its tuning/positions, and authors version any changed lesson anchor.

Authored palmMuted events carry a technique cue and synthetic reference decay through the shared MusicalEvent, TAB/staff and immutable snapshots. The renderer retains original per-voice onset age during seeks. The exercise validator restricts these events to displayOnly pending a validated muted-input assessment contract; the DSP/scoring algorithms are unchanged.

Single-note pickStroke and multi-string strum share a derived notation direction, while retaining separate validation/serialization contracts. Picking cues do not alter audio rendering or scoring; historical omitted fields retain canonical JSON.

## Authored pitch bends

`PitchBend` is an optional canonical-event value: semitone amount and event-relative rise/optional release ticks. The same value drives fretted-position constraints, notation, analytic reference phase integration, actual-tempo capability checks and phase assessment. `PitchContourTrace` separates periodic moving pitch from `SustainTrace` stable targets. Both use the existing analyzer worker and common normalized frame structure; collectors persist only the traces required by the selected exercise. `BendEvaluator` emits immutable per-phase evidence and v4 aggregate scoring. Views draw the frozen target/observation data and never estimate pitch.
...
`LegatoChain` extends a monophonic event with bounded same-string targets while keeping one initial attack. Resolvers validate every intermediate fret; shared notation/reference/evidence/assessment use the same relative ticks and cumulative pitches. A prepared phase integral keeps reference rendering bounded. Wider positioning regions support two-handed patterns but never override physical self-checks or chord span limits. New optional event/result fields and opt-in capability/assessment/coach versions preserve earlier encoded history.

### Harmonic sounding targets

`HarmonicNote` is optional Domain metadata. `MusicalEvent.soundingPitches(in:)` supplies nearest MIDI labels and `soundingFrequencies(in:)` supplies exact ideal partial frequencies. Ordinary fret arithmetic remains the instrument map; it must not be used to derive a harmonic's sound from a touch landmark. `visualTargets(in:)` adds ordinary/base/touch roles, shared by selection, preview, practice and text. Natural nodes remain fixed; artificial octave relocation validates both hands. Audio transport caches canonical frequencies before rendering; capture ownership and callback work are unchanged. Harmonic assessment reuses pitch/onset/sustain with opt-in result/capability versions, and frozen history/coach targets preserve the distinction between measured sound and unverified physical production.

### Unpitched muted-string references

`MutedStringAttack` is a bounded optional Domain event source with physical strings/direction and no frets or pitches. It is mutually exclusive with pitched event metadata and restricted to display-only exercises. `soundingPitches`/`soundingFrequencies`/`visualTargets` therefore return no pitched targets. Lesson resolution preserves the explicit attack; visual snapshots expose muted strings and text renders its unpitched identity. Staff engraving uses a separate percussion position rather than a fabricated MIDI pitch. Transport renders a deterministic sample-age-addressed noise burst and preserves chunk/seek/loop behavior; capture ownership and callback allocation rules are unchanged. The existing grading gate rejects pure and mixed muted-attack exercises.

### Independently held reference voices

`MusicalEvent.heldStrings` distinguishes ongoing physical voices from `attackedPositions`. Exercise validation requires contiguous identical preceding positions and a plain display-only contract. `referenceVoiceSpans` merges continuations into complete per-string lifetimes before audio rendering. Transport caches frequencies and uses original onset phase, final end envelope and a stable exercise-wide voice divisor; internal melody boundaries neither retrigger nor attenuate held bass. Capture, scoring and the real-time callback are unchanged. Learning rejects metadata-losing projections/scopes/relocation and preserves physical strings while transposing. TAB/accessibility differentiate held frets; simultaneous staff notation remains explicitly unsupported. Empty metadata preserves previous canonical encoding and archived digests.
